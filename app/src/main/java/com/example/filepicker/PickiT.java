    ...
    public void getPath(Uri uri, int APILevel){
        ...
        if (APILevel>=19){
            ...
            // Local file was selected
                ...
                returnedPath = FilePickerUtils.getRealPathFromURI_API19(context, uri);
                ...
                if (returnedPath == null || returnedPath.equals("")){
                    ...
                    if (!unknownProviderCalledBefore) {
                        ...
                        if (uri.getScheme() != null && uri.getScheme().equals(ContentResolver.SCHEME_CONTENT)) {
                            ...
                            if (FilePickerUtils.errorReason() != null && FilePickerUtils.errorReason().equals("dataReturnedNull")) {
                                isFromUnknownProvider = true;
                                //Copy the file to the temporary folder
                                downloadFile(uri);
                                return;
                            }else if (FilePickerUtils.errorReason() != null && FilePickerUtils.errorReason().contains("column '_data' does not exist")){
                                isFromUnknownProvider = true;
                                //Copy the file to the temporary folder
                                downloadFile(uri);
                                return;
                            }else if (FilePickerUtils.errorReason() != null && FilePickerUtils.errorReason().equals("uri")){
                                isFromUnknownProvider = true;
                                //Copy the file to the temporary folder
                                downloadFile(uri);
                                return;
                    ...
                    pickiTCallbacks.PickiTonCompleteListener(returnedPath, false, false, false, FilePickerUtils.errorReason());
                }
                // Path is not null
                else {
                    // This can be caused by two situations
            ...
            //Todo: Test API <19
            returnedPath = FilePickerUtils.getRealPathFromURI_BelowAPI19(context, uri);
            pickiTCallbacks.PickiTonCompleteListener(returnedPath, false, false, true, "");
        }
